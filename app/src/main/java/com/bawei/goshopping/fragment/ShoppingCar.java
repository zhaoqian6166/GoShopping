...
public class ShoppingCar extends Fragment {
      ...
      //接口回调得到条目中的点击状态的改变  刷新适配器
        adapter.setOnSelecte(new CarAdapter.SetSelect() {
            ...
            public void onSelect(int position, boolean isSelect) {
                cartItemList.get(position).isChecked=isSelect;
        ...
        //删除条目的接口回调
        adapter.setItemDelete(new CarAdapter.ItemDelete() {
            @Override
    ...
    void deleteGood(int productID){
      //  int productID=0;
        String url="http://169.254.217.5:8080/bullking1/deletepro?productID="+productID;
